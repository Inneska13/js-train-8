...
function findElementIndexes(arr, element) {
  ...
  const arrIndexOf = arr.indexOf(element);
  const arrLastIndexOf = arr.lastIndexOf(element);

  let [] = [arrIndexOf, arrLastIndexOf];
  return [arrIndexOf, arrLastIndexOf];
}
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  const arrCopyWithin = arr.copyWithin(target, start, end);
...
function customEvery(arr, condition) {
  ...
  const arrEvery = arr.every(condition);
  return arrEvery;
}
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  const arrFill = arr.fill(value, start, end);
  return arrFill;
}
...
function customSome(arr, condition) {
  ...
  const arrSome = arr.some(condition);
  return arrSome;
  // Повертаємо результат перевірки
}
